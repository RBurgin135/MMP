...
class SymmetricUnPadding2D(Layer):
    ...
    def call(self, inputs):
        if self.data_format is "channels_last":
            output = inputs[:, self.padding[0]:inputs.shape[1] - self.padding[1],
                     self.padding[2]:inputs.shape[2] - self.padding[3], :]
            return (output)
        elif self.data_format is "channels_first":
            output = inputs[:, :, self.padding[0]:inputs.shape[1] - self.padding[1],
                     self.padding[2]:inputs.shape[2] - self.padding[3]]
...
def build1D(dataset, channels=3, count=6, samplesize=-1, keep_percent=0.2, flip=False, activity_regularizer=None,
            inverse_activity_regularizer=None, activation_before=False):
